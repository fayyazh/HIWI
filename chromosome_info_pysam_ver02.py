...
def chromosome_count(rec_id, i,j):
    # s_range contains reference chromosome, ref. start, ref. end and strand.
    s_range = (record[i][4], record[i][0], record[i][1], dic_s[record[i][2]], "".join(record[i][3]))
    c.update([s_range])
...
def printing(endResult):
    with open(args.outfile, "w") as f:
        for entry in endResult:
            wr = ("%s\t%s\t%s\t%s\t%s\t%s" % (entry[0], entry[1], entry[2], entry[3], c[(entry[0], entry[1], entry[2], dic_s[entry[3]], entry[4])], entry[4]))
            f.write(wr + '\n')
